...
- [x] Cola confirmada por el owner, ejecutada en orden: harness (hecho) → KPIs + donut + página de detalle (hecho) → merge (hecho, con su confirmación) → después del merge: KAN-341 (colores en el resto de pantallas)
- [x] **MERGE HECHO: `feature/kan281-departamento-evangelismo` → `master`, commit `a67e130`.** Reporte de ramas aclarado con el owner primero (solo 3 ramas reales sin mergear existían, no las ~11 que reporté al principio por error -- las demás ya estaban mergeadas de sesiones previas). `master` local sincronizado con origin (2 commits de bitácora de Matías) antes de mergear. Build de producción verificado limpio post-merge. `feature/rediseno-hero-plano` y `kan-reenviar-invitacion` NO se tocaron (diferida / ya superada, respectivamente)
- [ ] Falta: deploy real a producción (el merge a master no lo hace solo) -- avisar a Matías o hacerlo cuando corresponda
- [ ] Falta: KAN-341 (colores en el resto de pantallas de Evangelismo) -- ahora que el merge ya se hizo, es lo próximo en la cola
