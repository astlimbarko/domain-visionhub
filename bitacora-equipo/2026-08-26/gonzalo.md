...
- [ ] **BLOQUEADOR sin resolver**: Gonzalo probó el botón en local y tampoco aparece (no es solo tema de producción/despliegue) -- sesión cortada ahí a pedido suyo, sin diagnosticar. Ver memoria KAN-263 para las hipótesis a revisar la próxima vez. NO mergear a master hasta resolver esto
- [ ] Falta (después de resolver el bloqueador): mergear `kan-reenviar-invitacion`, desplegar el front nuevo, y probar el botón en vivo antes de pasar KAN-263 a Finalizada
- [x] KAN-266 creado (nuevo, real, primera vez que se ve Parte B de KAN-252 en vivo): al entrar como Super Admin a /administracion apareció el modal "Actualiza tu membresía" (Teléfono/Ministerio) y falló al guardar (`telefono.png`) -- Super Admin no debería recibir este modal, sospecha en `fn_mi_actualizacion_membresia_pendiente` sin excluir SUPER_ADMIN. Sin diagnosticar ni tocar, solo documentado
