...
/**
 * Calendario a nivel Iglesia: mismo patrón que `CalendarioRed.tsx` (mismo
 * grid/diálogo/próximos), un nivel más arriba. Todo evento que el
 * Pastor/Supervisor crea acá queda "de la Iglesia" (sin `casa_de_paz_id` ni
 * `red_id`) y ya aparece automáticamente en el calendario de cada Red y cada
 * CdP de esa iglesia -- ver `fn_eventos_cdp`/`fn_eventos_red`
 * (`100_calendario_ambito_iglesia.sql`). Pedido del owner, 2026-08-04.
 */
export function CalendarioIglesia({ iglesiaId }: Props) {
  const hoy = new Date();
  ...
  const { data: tipos = [] } = useTiposEvento(iglesiaId);
  const { data: eventos = [], isLoading: cargandoEventos, isFetching: actualizandoEventos } = useEventosIglesia(iglesiaId, desde, hasta, filtroTipoId);
  const { data: proximos = [] } = useProximosIglesia(iglesiaId);
  const crearEvento = useCrearEventoIglesia(iglesiaId);
  const eliminarEvento = useEliminarEventoIglesia(iglesiaId);
  ...
  // por rol: cualquier tipo creable sirve para un evento "de toda la Iglesia".
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <section className="overflow-hidden rounded-2xl border border-border/60 bg-card lg:col-span-2">
          <TarjetaHeader icon={CalendarDays} color={MARINO} titulo="Calendario de la Iglesia" descripcion={`Eventos de ${nombreMes(anio, mes)} visibles en todas las Redes y Casas de Paz`} />
          <div className="p-4">
            {cargandoEventos ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          <section className="overflow-hidden rounded-2xl border border-border/60 bg-card">
            <TarjetaHeader icon={CalendarClock} color={VERDE} titulo="Próximos" descripcion="Eventos de Iglesia de los próximos días" />
