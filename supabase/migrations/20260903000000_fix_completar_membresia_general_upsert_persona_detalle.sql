-- VisionHub -- fn_completar_membresia_general habia perdido el upsert en
-- persona_detalle: el fix de KAN-213 (20260821040000, ON CONFLICT DO UPDATE
-- para que un reintento no chocara con uq_persona_detalle) quedo pisado
-- cuando 20260825160000 recreo la funcion entera para sumar telefono/
-- invitaciones, partiendo de una copia anterior a ese fix. Resultado: un
-- doble clic en "Finalizar" (dos requests casi simultaneos que pasan la
-- misma verificacion inicial) hace que el segundo choque con la fila de
-- persona_detalle que ya inserto el primero -- error real de base de datos
-- que el frontend, al no reconocerlo, muestra como "revisa tu conexion".
-- Se vuelve a poner ON CONFLICT, sin tocar nada mas de la version vigente.
