-- KAN-420 (2026-09-22, pedido explicito del owner): "Nuevo Convertido" es
-- conceptualmente solo para mayores de 12 -- fn_recalcular_estados_cdp_reporte
-- promovia SIM->NC automaticamente por cantidad de asistencias, sin mirar la
-- edad, asi que un nino que asistia 2+ veces quedaba clasificado como Nuevo
-- Convertido igual que un adulto. Regla de datos (no solo visual, decision
-- confirmada por el owner): nunca guardar estado=NC para un menor de 12 --
-- se queda en SIM (Simpatizante) indefinidamente via este mecanismo
-- automatico, sin bloquear que se le complete el resto de sus datos.
--
-- La edad se resuelve con fecha_nacimiento cuando existe, o edad_aproximada
-- (KAN-406) como respaldo cuando no -- igual criterio que ya usa el resto
-- del proyecto para "es menor" (ver BuscadorPersonaMultiple.tsx). Si ninguna
-- de las 2 esta cargada, se deja pasar (no se puede probar que es menor).
--
-- Mismo CREATE OR REPLACE que 20260917220000 (firma identica), solo se
-- agrega el chequeo de edad en la rama "Nunca fue NC: umbral configurable".
...
BEGIN
  ...
  -- Ciclo SIM<->CRE de miembros reales (sin cambios respecto a KAN-395).
  FOR v_miembro IN
  ...
  LOOP
    ...
    ELSIF v_estado_activo_id = v_estado_sim_id THEN
      ...
      IF v_fue_nc_alguna_vez THEN
        ...
        END IF;
      ELSE
        -- KAN-420: antes de promover a NC por cantidad de asistencias, se
        -- verifica que no sea un menor de 12 -- "Nuevo Convertido" es
        -- conceptualmente solo para mayores de esa edad. Se resuelve con
        -- fecha_nacimiento real o, en su ausencia, edad_aproximada
        -- (KAN-406). Si ninguna está cargada, se deja pasar (no se puede
        -- probar que es menor) -- mismo criterio que el resto del proyecto.
        SELECT (
          (p.fecha_nacimiento IS NOT NULL AND EXTRACT(YEAR FROM age(v_fecha_reunion, p.fecha_nacimiento)) < 12)
          OR (p.fecha_nacimiento IS NULL AND p.edad_aproximada IS NOT NULL AND p.edad_aproximada < 12)
        )
        INTO v_es_menor_12
        FROM public.persona p
        WHERE p.id = v_visita.persona_id;

        IF v_asistencias_desde_sim >= v_umbral_visitas AND NOT coalesce(v_es_menor_12, false) THEN
          UPDATE public.persona_estado
          SET fecha_fin = current_date
          WHERE persona_id = v_visita.persona_id AND estado_id = v_estado_sim_id
            AND fecha_fin IS NULL AND fecha_eliminacion IS NULL;
