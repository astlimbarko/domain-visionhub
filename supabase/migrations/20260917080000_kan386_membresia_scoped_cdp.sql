-- VisionHub -- KAN-386 seguimiento (2026-09-17, pedido explicito del owner):
-- Panel "Membresia" por Casa de Paz -- mismo censo que ya ve Supervision en
-- Afirmacion (fn_afirmacion_buscar_membresia), pero accesible para
-- Lider/Sublider de CdP y Lider/Supervisor de Red sobre SU CdP puntual.
--
-- Las 2 RPCs de Afirmacion (buscar membresia + estadisticas) hoy solo
-- permiten fn_es_lider_afirmacion_en/fn_es_operativo_en/fn_es_pastor_en/
-- fn_es_super_admin -- ningun Lider/Sublider de CdP ni Lider de Red pasa
-- ese gate, aunque la busqueda YA acepta p_casa_de_paz_id como filtro. Se
-- agrega un bypass angosto: cuando se pasa un p_casa_de_paz_id concreto,
-- tambien vale fn_puede_ver_cdp(esa_cdp) -- funcion ya existente que cubre
-- exactamente el alcance de roles confirmado (Lider/Sublider CdP, Lider de
-- Red de esa CdP, ademas de los que ya podian). No se toca nada del acceso
-- sin filtro (busqueda de toda la iglesia) que ya usa Afirmacion.
--
-- fn_afirmacion_estadisticas_personas no tenia forma de acotar a una CdP
-- -- se agrega p_casa_de_paz_id (default null, no rompe el llamado actual
-- de Afirmacion) y se filtra cada uno de los 5 bloques de conteo.

begin;

-- ============================================================
-- 1) fn_afirmacion_buscar_membresia: mismo cuerpo, solo el gate de permiso
--    gana un bypass para CdP puntual.
-- ============================================================

CREATE OR REPLACE FUNCTION public.fn_afirmacion_buscar_membresia(p_iglesia_id uuid, p_texto text DEFAULT NULL::text, p_pagina integer DEFAULT 1, p_por_pagina integer DEFAULT 50, p_red_id uuid DEFAULT NULL::uuid, p_casa_de_paz_id uuid DEFAULT NULL::uuid, p_estado_id uuid DEFAULT NULL::uuid, p_sexo sexo_enum DEFAULT NULL::sexo_enum, p_via_registro text DEFAULT NULL::text, p_con_profesion boolean DEFAULT NULL::boolean, p_estado_civil text DEFAULT NULL::text, p_bautizado boolean DEFAULT NULL::boolean)
...
-- ============================================================
-- 2) fn_afirmacion_estadisticas_personas: suma p_casa_de_paz_id opcional.
-- ============================================================
