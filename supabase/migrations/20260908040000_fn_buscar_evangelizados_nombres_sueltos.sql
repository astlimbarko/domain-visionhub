-- VisionHub -- rediseño de la tabla desktop de "Personas evangelizadas"
-- (pedido explícito del owner, 2026-09-08): nombre de la persona en 2 líneas
-- (nombres arriba, apellidos abajo) y nombre del evangelizador abreviado
-- según el largo de sus nombres. Ambos formatos se arman en el frontend, así
-- que la RPC necesita los pedazos sueltos (primer_nombre, segundo_nombre,
-- primer_apellido, segundo_apellido), no solo el `nombre_completo` ya
-- concatenado que devolvía hasta ahora.

DROP FUNCTION IF EXISTS public.fn_buscar_evangelizados(UUID, UUID, TEXT, DATE, DATE, INT, INT, UUID, UUID, UUID);
...
CREATE FUNCTION public.fn_buscar_evangelizados(
  ...
  p_evangelizado_por_id UUID DEFAULT NULL
)
RETURNS TABLE (
  id UUID,
  persona_id UUID,
  ...
  evangelizado_por_primer_nombre VARCHAR,
  evangelizado_por_segundo_nombre VARCHAR,
  evangelizado_por_primer_apellido VARCHAR,
  sexo TEXT,
...
REVOKE ALL ON FUNCTION public.fn_buscar_evangelizados(UUID, UUID, TEXT, DATE, DATE, INT, INT, UUID, UUID, UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.fn_buscar_evangelizados(UUID, UUID, TEXT, DATE, DATE, INT, INT, UUID, UUID, UUID) TO authenticated;
