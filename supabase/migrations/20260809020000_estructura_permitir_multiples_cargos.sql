-- VisionHub -- estructura_permitir_multiples_cargos
-- Decision del owner (Gonzalo, confirmada 2026-08-09 vía Matías): una
-- persona SI puede tener mas de un cargo de sistema a la vez en la misma
-- iglesia (ej. Pastor y Supervisor de la Vision en Accion juntos). Se saca
-- el guard ESTRUCTURA_PERSONA_YA_TIENE_ROL de fn_estructura_asignar_pastor
-- y fn_estructura_asignar_supervisor -- eran las unicas 2 funciones con esa
-- restriccion puntual (verificado: no existe en ninguna otra funcion ni en
-- los triggers de red_cargo/casa_de_paz_cargo, que enforcen una regla
-- distinta -- que una Red/CdP no tenga dos personas en el MISMO cargo, algo
-- que no cambia).
