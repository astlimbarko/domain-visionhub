-- VisionHub -- KAN-347: el owner pidio agregar el estado SSVA (SIM/NC/CRE)
-- al avatar de la tarjeta mobile (migracion 20260907050000), pero se
-- arrepintio: no es relevante para esta pantalla y ni siquiera se muestra en
-- la vista de escritorio. Se revierte esa columna/join. En su lugar el
-- avatar va a mostrar el tipo de evangelismo (1+1/Elite), que ya se
-- devuelve como tipo_evangelismo_nombre/color -- no hace falta agregar nada
-- nuevo para eso.
--
-- De paso, bug real encontrado: `tipo_evangelismo_color` estaba declarada
-- como `CHARACTER` a secas (= CHARACTER(1) en Postgres) en el RETURNS TABLE
-- desde que se creo esta funcion (20260906000000) -- el color hex de 7
-- caracteres se truncaba a un solo caracter ("#") al armar la fila de
-- salida, dejando el color del tipo de evangelismo roto en TODA la pantalla
-- (tabla de escritorio y tarjeta mobile) desde el dia 1. Se corrige a VARCHAR.
