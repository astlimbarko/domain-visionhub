-- VisionHub -- KAN-250 (pedido del owner, 2026-08-22): fn_buscar_personas
-- deja pasar a un Super Admin buscar personas de CUALQUIER iglesia del SaaS
-- (KAN-151, 2026-08-05) -- pensado en su momento para designar Pastor/
-- Supervisor desde el Constructor. El mismo bypass, sin querer, tambien
-- aplicaba al buscar candidatos para Lider/Supervisor de Red o Lider de CdP
-- (misma funcion, mismo parametro `v_global`, sin forma de distinguir desde
-- que pantalla se llama). El owner pide que ESE caso puntual (Red/CdP) quede
-- acotado a la iglesia propia, sin tocar Pastor/Supervisor (que sigue
-- pudiendo buscar en todo el SaaS, comportamiento intencional de KAN-151).
--
-- Se agrega p_permitir_global (default true, no rompe a quien no lo pasa --
-- Pastor/Supervisor/Personas/Afirmacion siguen igual) para que el frontend
-- lo desactive puntualmente en las 2 busquedas de PanelRedEstructura.tsx
-- (Lider/Supervisor de Red, y Lider de CdP al crear una Casa de Paz).
--
-- DROP + CREATE explicito con la firma EXACTA actual (7 parametros, fijada
-- hoy mismo en KAN-247/20260822200000) -- no un CREATE OR REPLACE, porque
-- agregar un parametro nuevo es tecnicamente una firma distinta para
-- Postgres y un CREATE OR REPLACE con distinta lista de parametros falla.
-- Un DROP con la firma que no coincide exactamente (mismo patron que causo
-- KAN-90/KAN-247) dejaria 2 versiones sueltas otra vez.
...
CREATE FUNCTION fn_buscar_personas(
  ...
  p_permitir_global BOOLEAN DEFAULT true
)
RETURNS TABLE (
...
BEGIN
  ...
  v_global := fn_es_super_admin() AND p_permitir_global;
...
GRANT EXECUTE ON FUNCTION fn_buscar_personas(UUID, TEXT, BOOLEAN, INT, BOOLEAN, INT, INT, BOOLEAN) TO authenticated;
