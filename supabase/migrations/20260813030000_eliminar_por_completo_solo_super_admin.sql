-- VisionHub -- KAN-190: eliminar por completo (borrado permanente e
-- irreversible, tanto de Casa de Paz como de Red) queda exclusivo de Super
-- Admin -- pedido explicito del owner (2026-08-13): "solo desde mi rol se
-- puede hacer eliminaciones asi arbitrarias, otros roles no podran (ej.
-- Supervisor de la Vision en Accion, Lider de Casa de Paz, Supervisor de
-- Red), todo por seguridad, ya que ellos no necesitan hacer eso -- solo yo".
-- Antes tambien podia Supervisor de la Vision en Accion
-- (fn_es_operativo_en). El resto de la funcion (chequeo de "vacia", borrado
-- en cascada, OTP) queda igual -- solo cambia el gate de quien puede
-- llamarla.
...
BEGIN
  ...
  IF NOT public.fn_es_super_admin() THEN
    RAISE EXCEPTION 'SOLO_SUPER_ADMIN: se requiere ser Super Admin para eliminar una Casa de Paz de la base de datos'
      USING ERRCODE = 'P0001';
  END IF;
...
BEGIN
  ...
  IF NOT public.fn_es_super_admin() THEN
    RAISE EXCEPTION 'SOLO_SUPER_ADMIN: se requiere ser Super Admin para eliminar una Red de la base de datos'
      USING ERRCODE = 'P0001';
  END IF;
