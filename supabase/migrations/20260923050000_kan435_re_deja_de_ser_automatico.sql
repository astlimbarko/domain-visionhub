-- KAN-435 (2026-09-23, pedido explícito del owner, revierte una decisión
-- de diseño confirmada en julio -- ver harness/05-estados-ssva/design.md,
-- sección "Riesgos y decisiones", fila "RE nunca sale de RE..." y
-- PENDIENTES.md #2 2026-07-17): hasta hoy, `fn_recalcular_estados_cdp_reporte`
-- reclasificaba automáticamente a RE a cualquier miembro que volviera a
-- asistir después de >= DIAS_PARA_RE (90) días sin venir, sin que ningún
-- humano lo confirmara.
--
-- Caso real que lo disparó: un miembro real volvió a asistir después de
-- mucho tiempo y quedó etiquetado RE (Reconciliado -- "se reconcilió con
-- la fe") solo porque pasó el umbral de tiempo, sin que el líder hubiera
-- dicho nada al respecto. El diseño original confundía dos cosas
-- distintas: "volvió después de mucho tiempo" (un dato objetivo, medible
-- por la computadora) con "se reconcilió con la fe" (una decisión
-- espiritual, que solo puede confirmar un humano).
--
-- Se saca por completo el criterio de tiempo. RE pasa a ser 100% manual:
-- el toggle que ya existe en el reporte de Casa de Paz (KAN-390/KAN-421,
-- Reportes.tsx) es ahora el único camino para marcar a alguien como RE,
-- sin importar cuánto tiempo llevaba sin asistir -- "si ya existe en el
-- sistema, basta con eso" para que un líder pueda marcarlo, pero nunca lo
-- hace el sistema solo.
--
-- Sin este cambio, un miembro que vuelve tras la ausencia sigue el mismo
-- ciclo SIM<->CRE de siempre (si estaba SIM, vuelve a CRE al asistir; si
-- ya estaba en cualquier otro estado -- incluido uno RE puesto a mano --
-- no se toca). El criterio DIAS_PARA_RE queda sin uso en el código (no se
-- borra de la tabla criterio_valor/criterio_definicion -- son datos
-- históricos de configuración, inofensivos sin código que los lea).
...
BEGIN
  ...
  -- Ciclo SIM<->CRE de miembros reales. KAN-435: ya NO hay reclasificación
  -- automática a RE por tiempo sin asistir -- RE es 100% manual (toggle en
  -- Reportes.tsx). Un miembro que ya estaba en RE (puesto a mano) se queda
  -- ahí sin cambios; uno que estaba SIM vuelve a CRE al asistir, igual que
  -- siempre.
  FOR v_miembro IN
