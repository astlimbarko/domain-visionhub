-- VisionHub -- KAN-367 (hallazgo colateral, 2026-09-17): fn_recalcular_estados_cdp_reporte
-- estaba declarada STABLE pero hace UPDATE/INSERT en persona_estado -- Postgres
-- rechaza mutaciones en funciones no-VOLATILE ("UPDATE is not allowed in a
-- non-volatile function"). Nunca estuvo en git (no aparecía en harness/ ni en
-- supabase/migrations/ hasta ahora), asi que quedó rota en silencio: el llamado
-- vive en un try/catch de mejor esfuerzo (reporte.service.ts) que nunca revierte
-- el reporte, solo lo dejaba de reclasificar Simpatizante/Creyente (KAN-183) sin
-- que nadie lo notara. Mismo cuerpo, unico cambio real es sacar STABLE (default
-- es VOLATILE).
