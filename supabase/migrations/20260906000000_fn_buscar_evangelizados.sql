-- VisionHub -- KAN-302 (pedido explicito del owner, 2026-09-06): pagina
-- "Personas evangelizadas" para Supervisor/Pastor/Departamento de Evangelismo,
-- mismo patron que AfirmacionPersonas.tsx (fn_buscar_personas): tabla completa
-- con filtros (texto, Red, rango de fechas), paginada, con boton de exportar
-- CSV -- en vez de un simple boton de exportar suelto.
--
-- Alcance iglesia completa (igual que fn_evangelismo_red / fn_metas_cdp_red):
-- Pastor, Supervisor, o Lider de Departamento de Evangelismo. El Lider/
-- Sublider de CdP y el Lider de Red ya tienen su propio listado acotado en
...
RETURNS TABLE (
  ...
  -- tipo_evangelismo.color es CHARACTER(7), no VARCHAR (mismo tipo que ya
  -- usa fn_evangelismo_red) -- declarar VARCHAR acá rompe con "structure of
  -- query does not match function result type" (bug real, encontrado en vivo).
...
BEGIN
  ...
  -- Columnas calificadas con "red." -- RETURNS TABLE declara "id" como
  -- parametro OUT visible en toda la funcion (variable PL/pgSQL), asi que
  -- una referencia sin calificar a "id" acá choca contra ese OUT y PL/pgSQL
  -- tira "column reference is ambiguous" (bug real, encontrado en vivo).
