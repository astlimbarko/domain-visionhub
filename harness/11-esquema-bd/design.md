...
| Una función `LANGUAGE plpgsql` con `RETURNS TABLE(col ...)` convierte cada columna de salida en una variable visible en toda la función. Una referencia **sin calificar** a una columna de tabla con el mismo nombre (`WHERE id = ...` cuando la tabla tiene `id` y el `RETURNS TABLE` también) es ambigua y PostgreSQL la rechaza en tiempo de ejecución, no al crear la función — pasa desapercibida hasta que alguien la llama con datos reales. Encontrado el 2026-07-18 en `fn_eventos_cdp` y `fn_cumpleanos_cdp` ([07-calendario-eventos](../07-calendario-eventos/)) al construir el frontend del calendario; ninguna prueba curl las ejercitaba. | Corregido calificando toda referencia con el alias de la tabla o de la CTE, en las dos funciones encontradas. **Sin auditar en el resto:** `05`, `08`, `09`, `10`, `11`, `12`, `14` y `17` también tienen funciones `plpgsql` con `RETURNS TABLE` — revisar cada una al construir el módulo de frontend que la use, mismo patrón que se viene siguiendo esta sesión. |
