...
| **Encontrado 2026-07-19, al construir la pantalla de Administración:** un `INSERT ... RETURNING` en `iglesia` (o el `.select()` de PostgREST equivalente) falla con `42501` para un Super Admin, aunque `fn_es_super_admin()` dé `true`. Causa: `fn_mis_iglesias()` vuelve a consultar `iglesia` por dentro para la rama Super Admin (`SELECT i.id FROM iglesia i WHERE fn_es_super_admin()`), y esa relectura dentro de la misma sentencia que todavía está insertando la fila nueva no la ve como visible a tiempo — es la única tabla del esquema donde `fn_mis_iglesias()` se referencia a sí misma. Un `INSERT` sin `RETURNING`/`.select()` funciona sin problema; una `SELECT` aparte después sí ve la fila. **Mitigación aplicada:** `crearIglesia()` en el frontend no pide `.select()` de vuelta. Pendiente si hace falta una solución de fondo: evaluar si la rama Super Admin de `fn_mis_iglesias()` necesita `SET LOCAL` para forzar bypass de RLS explícito en vez de depender del bypass implícito del dueño de la tabla. |
